...
public class CredentialController {
    ...
    public String createOrUpdateCredential(Credential credential, Authentication authentication, RedirectAttributes redirectAttributes) {
        ...
        if(credential.getCredentialId().intValue() > 0){
            try {
                ...
                redirectAttributes.addFlashAttribute("successMessage", "Your credentials were updated properly.");
                return "redirect:/home";
            } catch (Exception e) {
                logger.error("Error: " + e.getCause() + message + e.getMessage());
                redirectAttributes.addFlashAttribute("errorMessage", "Something happened wrong with the credentials update. Please try again!");
                return redirectPath;
            }
        } else{
            try {
                ...
                redirectAttributes.addFlashAttribute("successMessage", "Your credentials were created properly.");
                return redirectPath;
            }  catch (Exception e) {
                logger.error("Error: " + e.getCause() + message + e.getMessage());
                redirectAttributes.addFlashAttribute("errorMessage", "Something happened wrong while creating Credential. Please try again!");
                return redirectPath;
            }
        }
    }
    ...
    /** Method to generate a random encrypted key which will be saved in database
